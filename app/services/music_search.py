...
def _read_tags(path: str) -> Dict:
    ...
    def _parse_year(val: Optional[str]) -> Optional[str]:
        if not val:
            return None
        text = str(val)
    ...
    def _parse_explicit(val: Optional[str]) -> Optional[bool]:
        if val is None:
            return None
        ...
        if text in {"explicit", "yes", "true", "1", "y", "e"}:
            return True
        if text in {"clean", "no", "false", "0", "n", "c"}:
            return False
        return None
