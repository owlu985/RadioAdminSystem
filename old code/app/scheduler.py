...
def record_stream(stream_url, duration, output_file, config_file_path, show_id):
	...
	try:
		...
		from app.audio_analysis import analyze_recording
		from app.postprocess import handle_completed_recording
		...
		analysis_result, analysis_json_path = analyze_recording(final_audio_path)

		handle_completed_recording(
			show=current_show,   # explained below
			recording_path=final_audio_path,
		...
		if not show:
			logger.error(f"Show {show_id} not found; skipping post-processing.")
			return
	...
	try:
		...
		logger.warning(f"Silence analysis skipped: {e}")
...
def schedule_recording(show):
	...
	safe_show = (show.show_name or "Show").replace(" ", "_")
	output_file = os.path.join(show_folder,f"{safe_show}_{show.id}")
