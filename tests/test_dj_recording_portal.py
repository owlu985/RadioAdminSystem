...
def test_schema_upgrade_creates_access_code_table_for_existing_database(portal_app):
    app, _ = portal_app
    with app.app_context():
        DJRecordingAccessCode.__table__.drop(db.engine)
        assert "dj_recording_access_code" not in inspect(db.engine).get_table_names()

        ensure_schema(app, app.logger)

        assert "dj_recording_access_code" in inspect(db.engine).get_table_names()
